# Least-privilege service accounts for the isolated stack. No SA keys are ever
# downloaded; GKE nodes use the attached SA and workloads use Workload Identity.

# Node pool SA: only what a GKE node needs.
resource "google_service_account" "gke_node_sa" {
  account_id   = local.gke_gsa
  display_name = "StudEd GKE node service account (isolated stack)"
}
...
# Account ID matches what argocd-install.sh and external-secrets.yaml expect.
