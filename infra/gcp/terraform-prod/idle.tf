# Cost-cutting automation for the isolated stack: idle-scout.
# Cloud Scheduler fires the Cloud Run job hourly; if there was no HTTPS LB
# traffic in the last 2h it scales the GKE node pool to 0 nodes.

# Idle-scout identity: can only scale THIS cluster down and read metrics.
resource "google_service_account" "idle_scout_sa" {
  account_id   = local.idle_gsa
  display_name = "StudEd idle-scout (auto scale-down, isolated stack)"
}
...
resource "google_cloud_run_v2_job" "studed2_idle_scout" {
  name                = "studed2-idle-scout"
  location            = var.region
...
# --- Hourly trigger ---
resource "google_cloud_scheduler_job" "studed2_idle_check" {
  name      = "studed2-idle-check"
  schedule  = "0 * * * *"
  ...
  http_target {
    http_method = "POST"
    uri         = "https://${var.region}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${local.project_id}/jobs/${google_cloud_run_v2_job.studed2_idle_scout.name}:run"
    oidc_token {
...
resource "google_cloud_run_v2_job_iam_member" "idle_scout_invoker" {
  location = var.region
  project  = local.project_id
  name     = google_cloud_run_v2_job.studed2_idle_scout.name
