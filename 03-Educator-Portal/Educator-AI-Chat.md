...
- **`AIAssistantPanel`** docks on the **right side of the Puck wave editor**
  (`/educator/courses/{course}/lessons/{lesson}/waves/{wave}`). Opening it
  shrinks the editor canvas (flex layout — content is never dropped); a
  header toggle re-expands the editor.
...
- On completion, generated blocks appear with an **"Insert into editor"**
  action that converts them to Puck items (`agentBlocksToPuckItems`) and
  appends them to the live canvas; the existing Save flow persists them via
  `puckToWaveData`.
...
| `src/components/educator/AIAssistantPanel.tsx` | Chat panel UI |
| `src/components/puck-blocks/puck-config.tsx` | `agentBlocksToPuckItems` conversion |
| `src/routes/educator/_layout/...waves.$waveId.tsx` | Editor + docked panel layout |
...
- ChemViz / ElecSim / MechSim / Manim **Puck renderers** are not yet added;
  viz blocks generated by the agent currently degrade to text blocks in the
  editor.
