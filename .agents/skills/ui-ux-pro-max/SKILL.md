...
- **Color System**: OKLCH color space tokens in `src/styles/index.css`.
  - Parchment Background: `oklch(0.984 0.003 95)` (`#FAFAF7`).
  - Ink Foreground: `oklch(0.145 0 0)` (`#111827`).
  - Primary (Intelligent Blue): `oklch(0.55 0.22 264)` (`#2563EB`).
  - Secondary (Knowledge Violet): `oklch(0.58 0.22 295)` (`#8B5CF6`).
  - Success (Growth Green): `oklch(0.64 0.18 145)` (`#10B981`).
  - Gold (Leaderboards/Badges): `oklch(0.76 0.18 75)` (`#F59E0B`).
- **Typography**: Headings in `Instrument Serif` / `Instrument Serif Italic`, Body in `Inter` or `Noto Sans Sinhala` (language toggled).
- **Audio System**: Web Audio API zero-download sound synthesis (`playClickSound()`, `playSuccessSound()`, `playLevelUpSound()`).
- **Submodule Components**: Render interactive `Math-To-Manim`, `3Dmol.js`, `tscircuit`, and `Matter.js` physics blocks in `LearnBlockRenderer`.
