...
## StudEd AWS Infrastructure Context

> Applied when authoring or deploying AWS CDK constructs for StudEd.

- **Stack Location**: `infra/cdk/` or `infra/terraform/` integration.
- **Key Resources**:
  - **AWS ECR**: Microservice image repositories (`studed-api-gateway`, `studed-auth-service`, `studed-course-service`, `studed-progress-service`, `studed-gamification-service`, `studed-ai-service`, `studed-payment-service`, `studed-notification-service`).
  - **AWS S3 / Cloudflare R2**: Object storage for course media assets, Puck MDX blocks, and rendered Manim MP4 videos.
  - **AWS ECS / Fargate or EKS**: Containerized microservices deployment target.
  - **AWS RDS PostgreSQL / ElastiCache Redis**: Production managed database options.
