...
## StudEd Infrastructure Customization

> Applied when managing infrastructure for StudEd in `infra/terraform/`.

- **Providers**: AWS, Cloudflare (R2 bucket for video/MDX storage & CDN), Fly.io / Kubernetes.
- **State Backend**: Remote S3/Cloudflare R2 state with DynamoDB state locking.
- **Environments**: `staging` and `production` workspaces under `infra/terraform/environments/`.
- **Services Covered**: PostgreSQL 15, Redis 7, Elasticsearch 8, Cloudflare CDN assets.
