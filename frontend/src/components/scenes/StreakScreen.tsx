...
export function StreakScreen({ playKey, onNext, streakCount = 7, longestStreak = 12 }: { playKey: string | number; onNext?: () => void; streakCount?: number; longestStreak?: number }) {
  const soft = SOFT_EASE
  const reduce = useReducedMotion()
  return (
    <div key={String(playKey)} className="st">
      <motion.div className="st-bg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, ease: soft }} />
      <div className="st-stage">
          ...
          <div className="st-confetti" aria-hidden="true">
            ...
            {STREAK_CONFETTI.map((c, i) => (
              <motion.span
                ...
                transition={{ delay: c.delay, duration: 1.7, times: [0, 0.42, 1], ease: ['easeOut', 'easeIn'] }}
              />
            ))}
          </div>
        )}
        <motion.div className="st-flame" initial={{ scale: 0, rotate: -8 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: 0.15, duration: 0.6, ease: [0.34, 1.45, 0.5, 1] }}>
          ...
          <svg className="st-flame-svg" viewBox="0 0 24 24" aria-hidden="true">
            <defs>
              ...
              <linearGradient id="stf-core" x1="12" y1="9" x2="12" y2="21" gradientUnits="userSpaceOnUse">
                <stop stopColor="#ff9a3d" />
                <stop offset="1" stopColor="#e64a12" />
              </linearGradient>
              <linearGradient id="stf-shell" x1="12.006" y1="1.17" x2="12.006" y2="22" gradientUnits="userSpaceOnUse">
                <stop stopColor="rgba(255, 200, 110, 0.8)" />
                <stop offset="1" stopColor="rgba(255, 150, 66, 0.85)" />
              </linearGradient>
              <linearGradient id="stf-light" x1="12.006" y1="1.17" x2="12.006" y2="13.233" gradientUnits="userSpaceOnUse">
                <stop stopColor="#ffffff" />
                <stop offset="1" stopColor="#ffffff" stopOpacity="0" />
              </linearGradient>
              <filter id="stf-blur" x="-100%" y="-100%" width="400%" height="400%" filterUnits="objectBoundingBox" primitiveUnits="userSpaceOnUse">
        ...
        <motion.div className="st-num" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.3, duration: 0.6, ease: [0.34, 1.45, 0.5, 1] }}>
          <StreakCount reduce={!!reduce} streak={streakCount} />
        </motion.div>
        <motion.div className="st-cap" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.48, duration: 0.5, ease: soft }}>
          day streak
        </motion.div>
        <motion.div
          ...
          transition={{ opacity: { delay: 0.68, duration: 0.18 }, default: { delay: 0.68, duration: 0.55, ease: soft } }}
        >
          {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => {
            ...
            return (
              <div className="st-day" key={i}>
                <span className={isToday ? 'st-day-l font-bold text-primary' : 'st-day-l'} aria-hidden="true">{d}</span>
                <motion.span
                  className={isCompleted ? (isToday ? 'st-dot is-today' : 'st-dot') : (isToday ? 'st-dot is-today is-empty' : 'st-dot is-empty')}
                  initial={{ scale: 0 }}
                  animate={{ scale: isToday ? [0, 1.3, 1] : 1 }}
                  transition={{ delay: 0.78 + i * 0.055, duration: isToday ? 0.6 : 0.5, ease: isToday ? 'easeOut' : [0.34, 1.5, 0.5, 1] }}
                >
                  {isCompleted ? (
                    <svg viewBox="0 0 24 24" aria-hidden="true">
                      <path d="M5 13 l4 4 L19 7" fill="none" stroke="currentColor" strokeWidth="3.4" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  ) : (
                  ...
                  {isToday && isCompleted && !reduce && (
                    <motion.span
                      ...
                      transition={{ delay: 1.78, duration: 0.75, times: [0, 0.3, 1], ease: 'easeOut' }}
                      aria-hidden="true"
                    />
                  )}
                </motion.span>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ opacity: { delay: 1.45, duration: 0.18 }, default: { delay: 1.45, duration: 0.55, ease: soft } }}
        >
      ...
      <div className="st-content">
        <motion.button type="button" className="wc-btn st-btn" onClick={onNext} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.62, duration: 0.5, ease: soft }}>
          Keep it going
        </motion.button>
      </div>
    </div>
