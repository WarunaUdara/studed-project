...
export interface DepthCarouselProps {
  ...
  activeIndex?: number;
  onChange?: (index: number, item: { image?: string; alt?: string; content?: ReactNode }) => void;
  className?: string;
}
...
export function DepthCarousel({
  ...
  return (
    <div
      ref={rootRef}
      className={`relative flex h-full min-h-[460px] w-full cursor-grab touch-pan-y select-none items-center justify-center outline-none [perspective-origin:50%_50%] active:cursor-grabbing focus-visible:rounded-2xl focus-visible:outline-2 focus-visible:outline-primary/50 focus-visible:[outline-offset:4px] ${className}`.trim()}
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" ref={stageRef}>
        {data.map((item, i) => (
          <div
            key={i}
            className="absolute left-1/2 top-1/2 cursor-pointer overflow-hidden border border-border/60 bg-card shadow-[0_30px_60px_-20px_rgba(0,0,0,0.45),0_8px_20px_-10px_rgba(0,0,0,0.3)] [transform:translate(-50%,-50%)] [transform-origin:center] [will-change:transform,opacity,filter]"
            ref={(el) => {
          ...
          >
            {item.content ? (
              <div className="size-full overflow-hidden">{item.content}</div>
            ) : (
              <img
      ...
      {showControls && count > 1 && (
        <>
          <button
            type="button"
            className="absolute left-2 sm:left-4 top-1/2 z-[3000] grid size-10 -translate-y-1/2 place-items-center rounded-full border border-border/80 bg-background/80 text-foreground backdrop-blur-md transition-all hover:scale-105 hover:bg-background active:scale-95 shadow-md"
          ...
          <button
            type="button"
            className="absolute right-2 sm:right-4 top-1/2 z-[3000] grid size-10 -translate-y-1/2 place-items-center rounded-full border border-border/80 bg-background/80 text-foreground backdrop-blur-md transition-all hover:scale-105 hover:bg-background active:scale-95 shadow-md"
      ...
      {showIndicators && count > 1 && (
        <div
          className="absolute bottom-3 left-1/2 z-[3000] flex -translate-x-1/2 gap-2 rounded-full bg-background/70 px-3.5 py-2 backdrop-blur-md border border-border/40 shadow-xs"
          role="tablist"
          aria-label="Slides"
        >
          {data.map((_, i) => (
