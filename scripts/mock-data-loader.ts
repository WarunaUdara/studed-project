...
async function main() {
  ...
  // Python 10 Challenges Course
  const pythonId = await createCourse(educator, "Python 10 Challenges", "python-10-challenges", "G10");
  const pythonLessonId = await createLesson(educator, pythonId, "Programming Challenges", 1);
  await publishLesson(educator, pythonLessonId);

  const pythonWaves = [
    { title: "Challenge 1: Print Output", q: "Which built-in function outputs text to the console in Python?", a: "print" },
    { title: "Challenge 2: Modulo Operator", q: "Which operator calculates the remainder of division (used to check if even)?", a: "%" },
    { title: "Challenge 3: Reverse String", q: "Which slice expression reverses a string 's' in Python?", a: "s[::-1]" },
    { title: "Challenge 4: List Addition", q: "Which list method adds an element to the end of a list in Python?", a: "append" },
    { title: "Challenge 5: Loop Range", q: "What sequence of integers is generated by range(1, 5)?", a: "1, 2, 3, 4" },
    { title: "Challenge 6: Safe Dictionary Access", q: "Which dictionary method returns a default value if the key does not exist?", a: "get" },
    { title: "Challenge 7: Define Function", q: "Which keyword is used to define a function in Python?", a: "def" },
    { title: "Challenge 8: List Comprehension", q: "What is the output of [x*2 for x in [1, 2, 3]]?", a: "[2, 4, 6]" },
    { title: "Challenge 9: Exception Blocks", q: "Which keyword handles exceptions caught within a try statement?", a: "except" },
    { title: "Challenge 10: Import Modules", q: "Which keyword imports functions or packages in Python?", a: "import" },
  ];

  for (let i = 0; i < pythonWaves.length; i++) {
    const w = pythonWaves[i];
    const waveId = await createWave(educator, pythonId, pythonLessonId, w.title, i + 1, w.q, w.a);
    await publishWave(educator, waveId);
  }
  await publishCourse(educator, pythonId);
  console.log(`[mock] published Python 10 Challenges`);

  console.log(`[mock] enrolling student and completing a wave...`);
  await enroll(student, scienceId);
  await enroll(student, mathId);
  await enroll(student, pythonId);
